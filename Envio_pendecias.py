...
ARQUIVO_EXCEL = os.path.join(BASE_DIR, "Pendencias", "Pendencias.xlsx")
...
ARQUIVO_CORPO = os.path.join(os.path.dirname(ARQUIVO_EXCEL), "corpo_email.txt")
    ...
    for _, row in grupo.iterrows():
        tabela_html += f"""
            <tr>
                ...
                <td>{safe_str(row['Cargo'])}</td>
                <td>{safe_str(row['Departamento'])}</td>
                <td>{int(float(row['Dias S/Batida'])) if not pd.isna(row['Dias S/Batida']) else ""}</td>
            </tr>
        """
